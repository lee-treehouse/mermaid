...
import type { Info, Packet, Pie, Architecture, GitGraph, Radar, Treemap, Milestone } from './index.js';

export type DiagramAST = Info | Packet | Pie | Architecture | GitGraph | Radar | Milestone;
...
const initializers = {
  ...
  milestone: async () => {
    const { createMilestoneServices } = await import('./language/milestone/index.js');
    const parser = createMilestoneServices().Milestone.parser.LangiumParser;
    parsers.milestone = parser;
  },
} as const;
...
export async function parse(diagramType: 'milestone', text: string): Promise<Milestone>;
